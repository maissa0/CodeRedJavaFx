...
public class ServiceSuiviObj implements IService<SuivieObjectif>{
    ...
    public ServiceSuiviObj(){ connection = MyConnexion.getInstance().getCnx();
    }
    @Override
    public void add(SuivieObjectif suivieObjectif) throws SQLException {
        String sql = "insert into suivi_objectif (objectif_id, date_suivi, nouveau_poids,commentaire) values (?, ?, ?,?)";
        try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)){
            ps.setInt(1, suivieObjectif.getObjectif().getId());
            ps.setDate(2, new Date(suivieObjectif.getDate().getTime()));
